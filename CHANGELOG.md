...
* Added `BoomerAMG` and `GAMG` pressure preconditioners for distributed solves, with curated keywords forwarded to PETSc's options system and a `freeze` count that holds the preconditioner fixed between rebuilds (`GAMG` defaults to 25, `BoomerAMG` to 10), and `petsc_options` on `run!` for anything not exposed. The same string also configures PETSc's start-up, so options such as `-log_view` need no environment variable [#160](@ref)
* Distributed solves map `Cgs` to PETSc `cgs` and `DILU` to block-Jacobi ILU(0) with a one-time warning, so serial solver settings run unchanged on a distributed mesh; a type with no PETSc mapping can be named through `petsc_options`. `atol`, `rtol` and `itmax` reach PETSc exactly as they reach Krylov.jl, and `convergence` is only the outer-iteration target on both paths. A script run with plain `julia` and a `distribute`d mesh solves through PETSc on a single rank [#160](@ref)
