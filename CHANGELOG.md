...
* The CPU `AMG` setup builds each coarse operator with the same threaded Galerkin product that refreshes it, instead of a serial sparse matrix product, and reads the transfer operators' row layout directly: the hierarchy is bitwise unchanged and setup takes a third less time on 8 threads (motorBike pressure 681 ms to 458 ms)
