...
* `run!` with a negative `write_interval` (SIMPLE and PISO) no longer builds an output writer, which for VTK copied the mesh to the host and prepared its output strings, about 2.5 s per run on the motorBike case (GPU 500 iterations 17.2 s to 14.0 s). The equation assembly no longer clears matrix entries that it then overwrites, and the pressure copies, the k-omega eddy viscosity and the wall-function scratch reset run threaded [#160](@ref)
