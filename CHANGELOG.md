...
* Added `BoomerAMG` and `GAMG` pressure preconditioners for distributed solves, with curated keywords forwarded to PETSc's options system, and `petsc_options` on `run!` for anything not exposed. The same string also configures PETSc's start-up, so options such as `-log_view` need no environment variable [#160](@ref)
