...
STATE: BUILDING
STEP: P1-M18-S11 - CUDA-aware segfault root cause
HEAD: 9f9dec5e
BRANCH: HM/distributed-draft
GATE: julia --project=. -e 'using Pkg; Pkg.test()'
resume: read `probe_summary.txt` in the session scratchpad (`gpu_probe.sh` runs; ucxenv batch = `UCX_ERROR_SIGNALS="" UCX_MEMTYPE_CACHE=n` pre-set at launch, baseline batch = none, san = compute-sanitizer); if the pre-set batch is clean and the baseline is not, the fault is UCX's SIGSEGV handler and the cure is code that restores Julia's handlers or a documented launch line (R2 decides); then rerun `test_gpu.jl` n=1,2 on `dev/petscenv_conda_ompi`, record in `dev/telemetry/conda_cuda_petsc.md`, archive `AUDIT.md` (D77), close M18, continue M19
## implementer
...
## position
M1-M14, M17 closed (M12 superseded by M13). M18-M24 opened by D72 from the audit; M16 restated to measurement only (D76). Plans for all seven live in `dev/plans/`. M18-S1 landed (d91c67b8), S2-S10 landed (9f9dec5e, D78-D84, suite 12/12 at n=1,2); S11 in progress.
## evidence
