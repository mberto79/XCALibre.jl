...
STATE: IDLE
STEP: P1 exit gate (every P1 milestone closed; M32 D203)
HEAD: 4de3bf76
...
resume: user runs the phase close (`xcalibre-close`, direct invocation only); open finding: VTK `initialise_writer` costs ~3 s per `run!` even with `write_interval=-1` (D202), not yet a milestone
...
## position
All P1 milestones closed. motorBike 500 iterations: 8t 48.1 s, 8-rank MPI 54.1 s, GPU 17.2 s, 1t 137.6 s (`dev/telemetry/memory_scaling.md` § P1-M30-S6 close). Open finding: VTK writer initialisation ~3 s per run even when nothing is written (D202).
