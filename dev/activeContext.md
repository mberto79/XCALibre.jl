...
STATE: BUILDING
STEP: P1-M28-S9 lazy element view over the S7 containers
HEAD: 49c1ff23
...
resume: add `ElementRef{T,A}` in `src/Mesh/Mesh_0_types.jl` (getindex on the S7 containers returns it; `getproperty` reads one column, `convert`/`==`/`Setfield.setproperties` materialise), widen `cell::Cell{TF}` in the BC macro, `Discretise_3_boundary_conditions.jl` and `_scheme_source!` to a `Cell`-or-ref union, run `fa_check`-style asserts, then the gate chain `s9 cpu1 cpu8 2d gpu cpu1b 2db` and same-session `XENV=env_base` `cpu1 2d cpu1b 2db` (plan p1-m28 S9 bar, D171)
...
- Order: M28 (S9, then S8 only if S9 refused, then S5 gDiff fold, then S6 close), then M29, then M30 (D159). Commit and push each step.

## position
M28 S1-S4, S7 landed: per-field layout now in in-house containers, no StructArrays (S7). Correctness bitwise throughout; runtime 8t −13%, 1t −11% (D165). Compile +22% 1t, +28% 2D against a same-session base (base drifted to 18.0 s, compare same-session only); the rise is inference on the 8-load element built per access (D170).
