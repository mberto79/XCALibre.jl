...
- PETSc receives the matrix through a coordinate map built once, reading the owned values in place wherever they live; on a GPU the matrix, right-hand side and solution stay on the device, and a GPU run against a PETSc without device support stops at setup. Of the PETSc libraries matching the field precision, the one with the narrowest index type that addresses the global system is used.
