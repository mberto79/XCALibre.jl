...
- [-] **P1-M22-S8** WITHDRAWN (D117) — (from P1-M21-S1, D92) the cell kernels that run over ghost rows (`discretise!`, `green_gauss!`, `div!`, `inverse_diagonal!`, `H!`) take their range from `n_owned` on a `DistributedMesh`; the CSR stays square - mechanism: ghost values arrive by halo exchange, so their rows are never used - cost: none - verdict: residual hashes bitwise equal at n=2 and n=8, `check_ghosts` zero, n=8 per-iteration time recorded (ghosts are 2.9 percent of rows at n=8, 10.8 at n=64 on 5 mm); withdrawn if the time does not move.
