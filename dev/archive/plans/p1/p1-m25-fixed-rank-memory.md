...
- [x] **P1-M25-S4** per-case precompile recipe (user chose: recipe now, shipped precompile as P1-M26, D108) - DELIVERED: the documented recipe run literally cuts private at `iterations` 556 → 359 (−35 percent) and first-run compile 12.1 → 0.04 s at 10 mm n=4, hash unchanged, load time unchanged, package precompile 13 s; case-specific statements measured first (D106), generic-only subset gains nothing (D107). Telemetry § S4.
- [x] **P1-M25-S5** DELIVERED: docs build green in 141 s, 0 errors; guide states shared vs private per node, the heap-size-hint lever and its cost, and the per-case precompile recipe. Was: the per-node memory recipe in the distributed guide states the measured private and shared cost and the heap-size-hint lever with its measured time cost, so a user can size ranks per node - mechanism: documentation - cost: none - verdict: docs build green.
