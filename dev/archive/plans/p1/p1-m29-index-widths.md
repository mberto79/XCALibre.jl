...
- [x] P1-M29-S3 LANDED (D193; periodic Int32 meshes had Int64 matrices): periodic BC maps (`face_map`, `faceAddress1/2`, `i`/`j`) follow `TI`; one sweep of `src/` for `zeros(Int`, `Int64[`, `Int[`, `Vector{Int}` stored on a mesh, equation or solver struct. Blast radius: periodic cases. Bar: periodic tests bitwise.
- [x] P1-M29-S4 LANDED (D194): `integer_type=Int32` default for mesh readers, with a clear error when a per-process face count or matrix nnz exceeds `typemax(Int32)`; `Int64` stays selectable. Blast radius: every reader and the whole suite. Bar: full serial suite by file, docs build.
