...
- [x] **P1-M7-S1** Rename `reuse` to a name that says it freezes, keeping the old keyword accepted with a deprecation for one release - mechanism: the field sets `KSPSetReusePreconditioner`, so the name states the PETSc call's semantics - cost: none at runtime - verdict: the docstring's description of what happens between solves matches what `_maybe_reuse_pc!` does. DONE: renamed `freeze` (D38); no alias, never released (D41).
- [x] **P1-M7-S2** Retune the defaults to the measured optima and state the residual-quality cost in the docstring - mechanism: the optimum is where rebuild cost and staleness cross, which the sweep locates - cost: one probe row per candidate - verdict: strict - the solve converges to the same solution (Krylov sees the current matrix); objective - s/iter at the swept rank counts improves on the old default; band - final pressure residual after 100 iterations at a fixed rank count within 2x of freeze=1, since s/iter cannot see a stale hierarchy slowing outer convergence. DONE: GAMG 25, BoomerAMG kept at 10 (D39).
- [x] **P1-M7-S3** Move the AMG recommendation to `GAMG` and say why in one clause - mechanism: GAMG's coefficient-only update is valid because a non-refining mesh fixes the sparsity pattern - cost: documentation only - verdict: the recommendation cites the telemetry rather than asserting. DONE: docstrings and user guide (D40).
- [x] **P1-M7-S4** Document that AMG breaks rank invariance and Jacobi does not - mechanism: an AMG hierarchy is built from the local partition, which changes with rank count - cost: documentation only - verdict: a user comparing runs at two rank counts finds the caveat before filing it as a bug. DONE: user-guide note and both docstrings (D40).
