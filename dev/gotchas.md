...
- BFS tet meshes at 3, 4, 5 and 10 mm live in `~/Desktop/BFS_GRIDS/` (`bfs_unv_tet_<h>mm.unv`); parts are refused at load unless written at the current part format (3, `.xdm`, since P1-M23-S1), so any `.jls` part is obsolete and must be regenerated with `mem_probe.jl part`.
