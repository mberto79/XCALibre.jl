...
OpenFOAM's own numbers for the same case (1 core 238.87 s, 8 cores 81.66 s).
  julia --project=dev/komega -t N dev/komega/bench500.jl [iterations] [out] [i64|i32]
=#
...
t = tref[]
open(out,"w") do f
    @printf(f, "threads=%d blas=%d index=%s  iterations=%d  seconds=%.2f  ms/iter=%.1f\n",
        Threads.nthreads(), BLAS.get_num_threads(), IX, iters, t, 1000t/iters)
