...
mk(sol, rx) = SolverSetup(solver=sol, preconditioner=Jacobi(), convergence=1e-14, relax=rx, rtol=1e-10, itmax=5000)
...
io = open("dev/komega/eq_check_tight.txt.log","w"); rows = String[]
...
close(io)
open("dev/komega/eq_check_tight.txt","w") do f
