...
- [ ] **P1-M5 - distributed dependencies become optional** Decide, on a measurement rather than a principle, whether MPI and Metis stay hard dependencies. The cost to a serial user is measured first: install size, precompilation and load time. Only if that cost is material does the module's MPI surface move behind an extension. Exit: the measurement recorded and the decision taken, plus the refactor if it is warranted. Serves R4. Expected 2 steps if the cost is immaterial, 6 if it is not: `HaloExchange` names `MPI.Comm` and `Vector{MPI.Request}` in its fields, and every file in the module calls MPI, so the refactor is not small.
