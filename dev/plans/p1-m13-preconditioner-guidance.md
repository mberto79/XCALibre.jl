...
- [ ] **P1-M13-S4** map `IC0GPU` to bjacobi+icc and `ILU0GPU` to bjacobi+ilu, each warning once that it is a substitute (D62) - mechanism: the serial types' closest per-rank PETSc relatives, as for DILU (D50) - cost: none - verdict: suite green and a distributed run with each type completes.
