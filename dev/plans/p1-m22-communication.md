...
- S7: whether PETSc's device context uses one stream for the whole solve or per-object streams; settled by reading `src/sys/objects/device/impls/cupm` in the conda PETSc source or by `-device_context_stream_type` behaviour. If per-object, use `-device_context_stream_type default_blocking` and CUDA.jl's legacy stream instead of events.
- S6: the PISO inner loop repeats `Hv`/p/`∇p` per corrector; the counters budget is per SIMPLE iteration and PISO gets its own line.
