...
- [x] **P1-M25-S2** DELIVERED (D103): ranked rows are first-run compilation about 190 MB and PETSc.jl +102 MB per rank; see telemetry § S2. Was: loaded-package audit: modules and shared libraries present in a CPU worker after setup, and the private bytes added by `using` each heavy dependency alone in a fresh process - mechanism: measurement - cost: one script under five minutes - verdict: a ranked list of private MB per dependency in the same telemetry file; each cure step below names its row.
