...
- [-] **P1-M25-S3** GC memory target set per rank by XCALibre at `distribute` (user's option 1) - WITHDRAWN (D105): steady-state private 459 vs 472/500 baseline at +11-17 percent `run!` time; the user lever (`--heap-size-hint`, `JULIA_HEAP_SIZE_HINT`) moves to S5.
- [ ] **P1-M25-S4** UPPER BOUND MEASURED (D106): case-specific statements cut private 559 → 359 and compile 11.5 → 0.04 s; shipping needs size-free kernel types and a case-covering workload, pending user choice. Was: precompile statements for the distributed CPU path (user's option 3) - mechanism: `--trace-compile` on one 10 mm n=4 worker covering `distribute`, `assign`, `wrap_eqn` and `SIMPLE`; keep signatures over package types only (drop `Main.`), emit them from the package and PETSc extension - cost: longer package precompile, measured - verdict: private at `iterations` and `t_iter_s` at 10 mm n=4 (2 iterations) fall, residual hash `dbc3c69ab48b3394` unchanged, serial `using XCALibre` time within 10 percent, precompile time recorded.
- [ ] **P1-M25-S5** the per-node memory recipe in the distributed guide states the measured private and shared cost and the heap-size-hint lever with its measured time cost, so a user can size ranks per node - mechanism: documentation - cost: none - verdict: docs build green.
