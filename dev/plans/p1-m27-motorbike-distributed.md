...
- [ ] **P1-M27-S5** An unsupported solver on a `DistributedMesh` must error instead of silently solving per rank (D154): CSIMPLE, CPISO, SIMPLE-MRF, Godunov, FilmModel and the LES models never call `wrap_eqn`. Mechanism: the guard the module already uses for a GPU solve without a device PETSc - error at setup, pointing at the documented scope. Blast radius: setup only; no supported path changes. Verdict: each unsupported combination errors with the documented message at n=2, the distributed gate and the serial suite green.
