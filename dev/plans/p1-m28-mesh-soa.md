...
- [ ] P1-M28-S5 (after P1-M31; re-scope then, since `face_gDiff` may simply become a `FaceArrays` column) fold `face_gDiff` into `Face2D`/`Face3D` as last field with a positional constructor that derives it; drop the `Mesh2`/`Mesh3` field and type parameter and `update_face_gDiff!`; Laplacian reads `face.gDiff`; `_XDM_FORMAT` bumped. Any 9-argument (full-field) face constructor must RECOMPUTE gDiff, because `@set face.x = ...` in BlockMesher2D rebuilds through it and would otherwise keep a stale value. Blast radius: Laplacian, readers, parts. Bar: strict class, 8t time within noise of S4.
