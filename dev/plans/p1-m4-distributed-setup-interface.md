...
- [ ] **P1-M4-S3** `petsc_options` reaches `PETSc.initialize` as well as the Krylov solver - mechanism: one options string, consumed by whichever PETSc stage recognises each entry - cost: none - verdict: `-log_view` passed through `run!` takes effect with `PETSC_OPTIONS` unset. `-use_gpu_aware_mpi 0` is only testable against a CUDA PETSc, which `PETSc_jll` does not ship, so `-log_view` stands for the mechanism. WRITTEN, NOT YET VERIFIED.
