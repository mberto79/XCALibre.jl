...
- `patch_probe.jl` - per-rank face count of every patch of a partitioned mesh, and which rank leaves which patch empty; the cheapest way to pick a rank count that exercises the empty-patch path (10 mm step: `top` empties on rank 2 at n=6, `wall` on rank 2 and `top` on rank 3 at n=8). `PROBE_GRID` names another grid in `examples/0_GRIDS` or an absolute path, `PROBE_SCALE` its scale. Run: `<MPI.mpiexec()> -n <n> julia --project=dev/petscenv_stock dev/scripts/patch_probe.jl`.
...
- `compile_sizes.jl` - compile cost of a second mesh size in one session (laminar SIMPLE on two box meshes, then the first again). Run: `julia --startup-file=no --project=dev/petscenv_stock dev/scripts/compile_sizes.jl`.
- `motorbike_smoke.jl` - 20-iteration residual history, field hashes, compile and run time for the motorBike (cpu, gpu, mpi) or 2D BFS kwSST (2d) case; `part` partitions offline; run: `julia --project=~/.cache/xcal_m28/env -t N dev/scripts/motorbike_smoke.jl cpu 20 <out>` (mpi via `mpiexecjl -n N ... mpi 20 <out> <partsdir>`).
