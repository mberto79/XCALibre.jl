...
# `pre=<Package>` loads a local package of harvested precompile statements before setup
const PRE = let i = findfirst(startswith("pre="), ARGS); i === nothing ? "" : ARGS[i][5:end] end
...
elseif MODE == "worker"
    ...
    isempty(PRE) || @eval using $(Symbol(PRE))
