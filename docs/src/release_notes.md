...
### Added
* `run!` takes a `progress` keyword (default `true`); `progress=false` turns off the progress bar, which we recommend for large-scale runs that are not on a local PC, such as cluster batch jobs [#160](@ref)
...
* Global cell and face ids of a distributed mesh (`Partition.local_to_global`, `orig_cells`, `orig_faces`) are `Int64` whatever the mesh index type, so an `Int32` part of a mesh with more than 2^31 cells or faces keeps exact global ids. Part files move to format 5; parts written earlier are refused at load and must be regenerated with `partition_mesh` [#160](@ref)
