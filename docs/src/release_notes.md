...
* Added FixedHeatFlux boundary condition [#149](https://github.com/github.com/mberto79/XCALibre.jl/issues/149)
* Added AMG-preconditioned stabilized biconjugate gradient for solving non-symmetric equations/ [#150](https://github.com/github.com/mberto79/XCALibre.jl/issues/150)
...
### Fixed
* Fixed serial linear solves stopping long before the requested `rtol` was reached. Krylov.jl measures its relative test on the preconditioned residual, which a few cells with a tiny diagonal can dominate: on a 10M-cell motorBike mesh the Jacobi-preconditioned pressure CG often stopped after 5 to 10 iterations (instead of about 180) with `||b - Ax||` barely reduced, and the steady run diverged after 100 iterations on CPU and GPU alike. `rtol` is now the reduction of the unpreconditioned residual, `||b - Ax|| <= rtol*||b - Ax0||`, for `Cg`, `Bicgstab`, `Cgs` and `Gmres`; `atol` keeps its meaning (an absolute bound on the preconditioned residual), so settings with `rtol = 0` behave exactly as before. `Gmres` with `rtol > 0` is right-preconditioned, and its `atol` then applies to the unpreconditioned residual too
...
* Fixed missing density term in set_production! needed for cases with non-unity density [#151](https://github.com/github.com/mberto79/XCALibre.jl/issues/151)
...
## Version [v0.6.0](https://github.com/github.com/mberto79/XCALibre.jl/releases/tag/v0.6.0) - 2026-07-17
