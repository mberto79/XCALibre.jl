...
* `activate_multithread(backend::CPU)` now defaults `nthreads` to `Threads.nthreads()` rather than 1. Krylov.jl sends every dot and axpy on a `Vector{Float64}` to BLAS, so this setting also decides how those are threaded [#161](@ref)
