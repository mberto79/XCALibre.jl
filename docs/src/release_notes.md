...
### Fixed
* Fixed warm-started serial `Bicgstab` solves degrading as a steady run converges. Krylov.jl's default shadow vector is `b`, which fits only a zero initial guess; every serial solve starts from the previous solution, and as `b - Ax0` shrank and turned away from `b` the iterates worsened (a 10M-cell motorBike LUST run on CPU drifted from about iteration 40 and diverged). The shadow vector is now `M⁻¹(b - Ax0)`, PETSc `bcgs`'s choice, so serial and distributed solves take the same iterates
