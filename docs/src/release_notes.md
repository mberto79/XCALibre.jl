...
* Removed the unused `xmul(y, A, x)` and `xmul!(A, x)` methods, which could not run, and replaced the broken `SparseMatrixCSR * SparseXCSR` product with a threaded `SparseXCSR * vector` product [#160](@ref)
* On the CPU, the Krylov solvers' vector operations, the sparse matrix-vector product and the Jacobi preconditioner run on Julia's threads over one fixed row split instead of in OpenBLAS, so the vector half of each linear solve scales with `-t`; operations on fewer than 2^16 elements stay serial. The 8-thread motorBike benchmark runs 13% faster; residuals move only by summation order [#160](@ref)
* Periodic boundary conditions keep an `Int32` mesh's equation matrix at `Int32`; their connectivity was built with `Int` and widened the matrix to `Int64` [#160](@ref)
* The AMG hierarchy stores its matrix, diagonal and aggregation indices in the index type of the equation matrix, so an `Int32` mesh gives an `Int32` hierarchy. `_workspace` takes that index type as a third argument [#160](@ref)
* Global cell and face ids of a distributed mesh (`Partition.local_to_global`, `orig_cells`, `orig_faces`) are `Int64` whatever the mesh index type, so an `Int32` part of a mesh with more than 2^31 cells or faces keeps exact global ids. Part files moved to format 5 (6 since the fix below); parts written earlier are refused at load and must be regenerated with `partition_mesh` [#160](@ref)
...
### Fixed
* Fixed CPU linear solves failing with "`@threads :static` cannot be used concurrently or nested" when `run!` or a Krylov solve is called from inside another `Threads.@threads` loop or from concurrent tasks; the threaded vector operations fall back to dynamic scheduling over the same chunks there, with unchanged results [#160](@ref)
* Fixed `AutoTune()` launches over an empty range failing with a `DivideError` or `ArgumentError` [#160](@ref)
* Fixed distributed runs hanging when one rank fails: an error in the mesh reader or partitioning on rank 0, a missing or unreadable part, a missing restart field or a broken `processor<rank>` directory now raises an error on every rank, naming the ranks that failed. `distribute(FOAMCase(dir))` also checks that `cellProcAddressing` matches the mesh beside it [#160](@ref)
* Fixed `distribute(reader; dir)` silently reusing another mesh's parts left in `dir`: parts record the mesh they were cut from and an optional `key`, `distribute(reader; dir, key)` repartitions when the key differs, and parts of different meshes mixed in one directory are refused at load. Part files move to format 6 and must be regenerated [#160](@ref)
* Fixed mesh readers failing with an unrelated error, or writing out of bounds, when a connectivity total exceeds the mesh integer type: totals are checked before they are narrowed, and the error asks for `integer_type=Int64` [#160](@ref)
