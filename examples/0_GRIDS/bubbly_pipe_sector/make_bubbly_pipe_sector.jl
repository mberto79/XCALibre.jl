...
# Case selection
# ----------------------------------------------------------------------------
#  ADIABATIC AIR-WATER BUBBLY UPFLOW
#
#  Adapted from `make_lh2_pipe_sector.jl` - the O-grid machinery below is
#  unchanged; only the geometry, fluid and development length differ. Generated
#  for VALIDATION of the interfacial force balance (lift, wall lubrication,
#  turbulent dispersion) against measured radial void profiles, with no boiling
#  and no phase change anywhere in the case.
#
#  Why an adiabatic case earns its place: the radial void distribution in a
#  vertical bubbly pipe is set entirely by the balance of lift, wall lubrication
#  and turbulent dispersion. Validating it here isolates that balance from the
#  wall boiling closure completely - so a disagreement has exactly one possible
#  cause, which is not true of any boiling case.
#
#  `:LiuBankoff` reproduces the geometry of Liu & Bankoff (1993), Int. J. Heat
#  Mass Transfer 36:1049, who documented the transition from a WALL-peaked to a
#  CORE-peaked void profile as bubble size increases - the measurement that
#  tests the sign change in the Tomiyama lift coefficient.
#
#  `:MTLOOP` is the FZD wire-mesh geometry (Lucas, Krepper & Prasser), the
#  dataset most CFD codes validate this force set against.
const CASE = :LiuBankoff

const D, L_meas = if CASE === :LiuBankoff
    38.1e-3, 10.0*38.1e-3      # measurement section, 10 D
elseif CASE === :MTLOOP
    51.2e-3, 10.0*51.2e-3
else
    error("Unknown CASE: $CASE")
end

# The generator names the middle section `pipeWall` and the two outer sections
# `wallUnheated`. Here NEITHER is heated - the split is kept only because it
# puts a patch boundary at a known axial station, which is where the radial
# profile is sampled.
const L_heated = L_meas
...
# Water at 20 C, 1 atm. `U_bulk` is the LIQUID superficial velocity j_l, which
# is what sets the friction velocity and therefore the near-wall cell height.
const U_bulk = 1.0           # [m/s]      j_l; Liu & Bankoff span 0.376 - 1.391
const rho_l  = 998.2         # [kg/m^3]   water at 20 C
const mu_l   = 1.002e-3      # [Pa s]

# WALL-RESOLVED, deliberately below the 30-50 wall-function band.
#
# This mesh exists to validate the near-wall bubble layer, and that layer is
# ONE BUBBLE RADIUS thick: `d_b/2 = 1.35 mm` for the 2.7 mm bubbles of the
# j_g = 0.112 case. Two closures live entirely inside it - the Shaver &
# Podowski lift damping, which is identically zero for `y < d_b/2`, and
# Lubchenko et al. Eq. 26, which is zero OUTSIDE it. Neither can shape a
# profile it has one sample point in.
#
#   y+ = 40  ->  h_wall = 1.52 mm  ->  1 cell inside d_b/2, centre at y/d_b = 0.30
#   y+ = 8   ->  h_wall = 0.30 mm  ->  4 cells inside d_b/2
#
# MEASURED on the coarse mesh, z/D = 36 against Liu & Bankoff Fig. 3(a1):
# the peak sat at r/R = 0.958 (y/d_b = 0.30) against a measured 0.903
# (y/d_b = 0.68), because the peak lands in the only cell available to it.
# Eq. 20 puts it at y = R_b, i.e. y/d_b = 0.50 - which on the coarse mesh falls
# BETWEEN the first two cell centres (0.80 and 2.32 mm) and cannot be represented
# at all.
#
# 0.30 mm also matches the reference: Lubchenko et al. (2018) use a 0.32 mm wall
# cell with a 1 mm base and 1.1 growth, and their mesh study spans 0.32-1.5 mm.
#
# The cost is that the first cell centre sits at y+ = 7, inside the viscous
# sublayer. That is the correct place for it here and the wall functions handle
# it - `nut_wall` returns exactly zero below `yPlusLam = 11.53`, and the omega
# blend `sqrt(w_vis^2 + w_log^2)` reduces to the viscous branch - but it does
# mean the wall layer is resolved rather than modelled, and cells 3-5 (y+ = 35,
# 49, 63) carry the log-layer behaviour instead of cell 1.
const y_plus_target = 8.0

# ----------------------------------------------------------------------------
# Mesh controls
# ----------------------------------------------------------------------------
# 40 D of development before the measurement section. Bubbly pipe data is taken
# at fully-developed conditions and the radial profile is what is being
# validated, so this is not a place to economise.
const dev_length_factor = 40.0   # inlet development length, in tube diameters
...
# `t_outer = n_outer*h_wall_target` of UNIFORM cells, so this sets how far the
# resolved layer extends. 5 x 0.304 mm = 1.52 mm, which covers `d_b/2 = 1.35 mm`
# with four cell centres inside it (y/d_b = 0.056, 0.169, 0.281, 0.394) and the
# fifth straddling the boundary at 0.507.
const n_outer           = 5      # cells across the OUTER ring (the wall layer)
...
println("""
=============================================================================
 Air-water bubbly upflow - 90 degree O-grid sector (ADIABATIC)
=============================================================================
 Case                : $CASE
 Inner diameter D    : $(D*1e3) mm
 Measurement len L   : $(L_heated*1e3) mm     (L/D = $(round(L_heated/D, digits=2)))
 Development length  : $(round(L_dev*1e3, digits=2)) mm  ($dev_length_factor D)
 Exit run            : $(round(L_exit*1e3, digits=2)) mm  ($exit_length_factor D, unheated)
...
# The 30-50 band applies to a WALL-MODELLED mesh. This one is deliberately
# resolved (see `y_plus_target`), so the check is on the resolved requirement
# instead: enough cells inside one bubble radius for the near-wall closures to
# have a profile to act on.
const _d_b_ref = 2.7e-3          # j_g = 0.112 case, Liu & Bankoff Fig. 12
const _n_in_bubble = count(i -> (i - 0.5)*h_wall_axis < 0.5*_d_b_ref, 1:n_outer)
if _n_in_bubble < 3
    @warn """Only $(_n_in_bubble) cell centre(s) fall inside one bubble radius \
(d_b/2 = $(round(0.5*_d_b_ref*1e3, digits=2)) mm) at h_wall = \
$(round(h_wall_axis*1e3, digits=3)) mm.

The Shaver & Podowski lift damping is zero for y < d_b/2 and Lubchenko Eq. 26 is \
zero outside it, so both act only in this layer. Fewer than ~3 cells there and \
the void peak lands in whichever cell happens to be first, regardless of the \
closures. Reduce `y_plus_target` or raise `n_outer`."""
end
if yplus_axis > 30
    @info "Wall-MODELLED spacing (y+ = $(round(yplus_axis, digits=1))); the near-wall bubble layer will be under-resolved."
end
