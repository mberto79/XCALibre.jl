...
const n_radial          = 9      # cells from core edge to wall
...
# With STRAIGHT core edges the ring is not a true annulus. Its three vertices sit
# on the circle r_i, so the thickness is exactly R - r_i there, but the chord
# between them sags inward to r_i*cos(22.5 deg), making the ring THICKER
# mid-edge. One grading over a varying thickness gives a varying wall cell
# height, so both limits must be reported honestly.
#
# Arcs on the core boundary (edges 1-2 and 2-3) would collapse these to a single
# value; see the `edges` section.
const t_min = R - r_i                    # at the block corners
const t_max = R - r_i*cosd(22.5)         # mid-chord, where the edge sags in

const t_axis = t_min    # retained names, now the two genuine limits
const t_diag = t_max
const t_mean = (t_min + t_max)/2

const radial_grading = grading_for_last_cell(t_mean, n_radial, h_wall_target)
...
const h_wall_axis = wall_cell_height(t_axis, n_radial, radial_grading)
const h_wall_diag = wall_cell_height(t_diag, n_radial, radial_grading)
...
const xy = [
    ...
    (r_i,  0.0),    # 1  v1  core/ring interface, on the x axis   |v1| = r_i
    (r_c,  r_c),    # 2  v2  core corner, r_c = r_i/sqrt(2)  ->   |v2| = r_i
    (0.0,  r_i),    # 3  v3  core/ring interface, on the y axis   |v3| = r_i
    (R,    0.0),    # 4  v4  wall at 0 deg
    (c45,  c45),    # 5  v5  wall at 45 deg
    (0.0,  R),      # 6  v6  wall at 90 deg
]
...
const blocks_xy = [
    ...
    # EAST: local axis 1 runs v1 -> v4, i.e. radially outwards. Graded.
    ((1, 4, 5, 2), n_radial, n_core,   (radial_grading, 1.0)),
    # NORTH: local axis 1 runs v3 -> v2 (azimuthal), axis 2 runs v2 -> v5
    #        (radially outwards). Graded on axis 2.
    ((3, 2, 5, 6), n_core,   n_radial, (1.0, radial_grading)),
]
...
(""")
        ...
        for iz in 0:(length(z_levels) - 1)
            z = z_levels[iz + 1]
            # OUTER arcs only. The core boundary is left STRAIGHT: its three
            # vertices still sit on the circle of radius r_i, so the ring
            # thickness is exact AT the vertices, but the chord between them
            # sags inward by (1 - cos(22.5 deg)) = 7.6% of r_i, making the ring
            # slightly thicker mid-edge. That residual variation is the price of
            # straight internal edges; arcs on (1,2) and (2,3) would remove it.
            for (a, b, mid_deg, rad) in ((4, 5, 22.5, R), (5, 6, 67.5, R))
                mx = rad*cosd(mid_deg)
                my = rad*sind(mid_deg)
        ...
        # Inlet: bottom faces of all three cross-section blocks, at z = 0.
        ...
        # block. Both are the outer arc: EAST face v4->v5, NORTH face v5->v6.
        emit_patch("wallUnheated", "wall",
                   [side_face(4, 5, 1), side_face(5, 6, 1)])
        emit_patch("pipeWall", "wall",
                   [side_face(4, 5, 2), side_face(5, 6, 2)])

        # Symmetry plane y = 0 (the x-z plane): CORE edge v0->v1, EAST edge v1->v4.
        sym_y = vcat([[side_face(0, 1, iz), side_face(1, 4, iz)] for iz in 1:2]...)
        emit_patch("symmetryY", "symmetry", sym_y)

        # Symmetry plane x = 0 (the y-z plane): CORE edge v3->v0, NORTH edge v6->v3.
        sym_x = vcat([[side_face(3, 0, iz), side_face(6, 3, iz)] for iz in 1:2]...)
...
=============================================================================
 ...
 Near-wall resolution
   ...
   achieved y+       : $(round(yplus_diag, digits=1)) - $(round(yplus_axis, digits=1))
   radial grading    : $(round(radial_grading, digits=5))  (last/first, < 1 = refine to wall)
