#!/usr/bin/env bash
# =============================================================================
#  Run blockMesh for the LH2 pipe sector from a path that contains spaces.
# =============================================================================
#
...
#      ./run_blockMesh.sh
#
#  Requires blockMesh on PATH (source your OpenFOAM bashrc first) and
#  system/blockMeshDict, which is produced by:
#
...
if [ ! -f "${CASE_DIR}/system/blockMeshDict" ]; then
    ...
    echo "       Generate it first:  julia make_lh2_pipe_sector.jl" >&2
    exit 1
fi

WORK_DIR="$(mktemp -d /tmp/lh2_pipe_sector.XXXXXX)"
...
# Mesh quality is worth looking at for an O-grid: the core/outer block junction
# at the 45 degree diagonal is where non-orthogonality concentrates.
...
echo "    julia --project=. test/unit_test_lh2_pipe_sector_mesh.jl"
