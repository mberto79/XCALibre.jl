...
# ─── GPU-native Galerkin update via oneMKL SpGEMM ────────────────────────────
# Requires oneMKL sparse×sparse multiplication support (oneAPI.jl ≥ 0.3).
# If `*` is not yet bound for oneSparseMatrixCSR, this will throw a MethodError
# and the hierarchy should instead be rebuilt with the CPU fallback path.
