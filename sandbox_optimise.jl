...
1:length(cell_faces_range)
cell_faces_range[5]
1:length(cell_faces_range[5])
faces=cell_faces[cell_faces_range[1]]
face=cell_faces[cell_faces_range[1]][1]

cell_neighbours=[]
for ID=1:length(cell_faces_range)
    for i=cell_faces_range[ID]
        for ic=1:length(i)
            index=findall(x->x==cell_faces[cell_faces_range[ID]][ic],cell_faces)
            #if length(index)==2
                #if i[1]<=index[1]<=i[end]
                    for ip=1:length(cells)
                        if cell_faces_range[ID][1]<=index[1]<=cell_faces_range[ID][end]
                            push!(cell_neighbours,ID)
                        end
                    end
                #end
                #if i[1]<=index[2]<=i[end]
                    for ip=1:length(cells)
                        if cell_faces_range[ID][1]<=index[2]<=cell_faces_range[ID][end]
                            push!(cell_neighbours,ID)
                        end
                    end
                #end
...
#
function generate_cell_neighbours_update(cell_faces,cells)
    cell_neighbours=Int64[]
    for ID=1:length(cells) 
        for i=cells[ID].faces_range 
            faces=cell_faces[i]
            for ic=1:length(i)
                ...
                if length(index)==2
                    if i[1]<=index[1]<=i[end]
                        #for ip=1:length(cells)
                            if cells[ID].faces_range[1]<=index[2]<=cells[ID].faces_range[end]
                                push!(cell_neighbours,ID)
                            end
                        #end
                    end
                    if i[1]<=index[2]<=i[end]
                        #for ip=1:length(cells)
                            if cells[ID].faces_range[1]<=index[1]<=cells[ID].faces_range[end]
                                push!(cell_neighbours,ID)
                            end
                        #end
                    end
...
@time generate_cell_neighbours_update(cell_faces,cells)
@time generate_cell_neighbours(cells,cell_faces)
...
cell_face_index=findall(x->x==cell_faces[1],cell_faces)
...
for i=1:length(cell_faces)
    cell_face=cell_faces[i]
    next_index=findall(x->x==cell_face,cell_faces)
    next_index=first(filter(x->x!=i,next_index))
...
typeof(findall(x->x==next_index,collect(cell_faces_range[5])))
cell_neighbours
collect(cell_faces_range[5])
cell_face
next_index
...
function generate_cell_neighbours_1(cell_faces,cell_faces_range)
    cell_neighbours=zeros(Int64,length(cell_faces))
    for i=1:length(cell_faces)
        next_index=filter(x->x!=i,findall(x->x==cell_faces[i],cell_faces))[1]
        for ic=1:length(cell_faces_range)
            if findall(x->x==next_index,collect(cell_faces_range[ic])) != Int64[]
                cell_neighbours[i]=ic
            end
...
@time generate_cell_neighbours_1(cell_faces,cell_faces_range)
...
#
cell_neighbours=Vector[]
#for i=1:length(cell_faces)
    cell_face=cell_faces[1]
    next_index=findall(x->x==cell_face,cell_faces)
    next_index=filter(x->x!=1,next_index)[1]
    for ic=1:length(cell_faces_range)
        if findall(x->x==next_index,collect(cell_faces_range[ic])) != Int64[]
