...
@inline dirichlet(term::Divergence{Linear}, A, b, cellID, cell, face, value) = begin
    A[cellID,cellID] += term.sign[1]*(term.J⋅face.normal*face.area)/2.0
    b[cellID] += term.sign[1]*(-term.J⋅face.normal*face.area)/2.0
    nothing
end
...
@inline neumann(term::Divergence{Linear}, A, b, cellID, cell, face, value) = begin
    ...
    b[cellID] += value + 0.0*term.sign[1]*(-term.J⋅face.normal*face.area)/2.0
    nothing
end