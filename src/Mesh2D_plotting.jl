...
function plot(p::Point{F}; colour=:blue) where F
    Plots.scatter([p.coords[1]], [p.coords[2]], color=colour, legend=false)
end

function plot!(fig, p::Point{F}; colour=:blue) where F
    Plots.scatter!(fig, [p.coords[1]], [p.coords[2]], color=colour, legend=false)
end

function plot(vec::Vector{Point{F}}; colour=:blue) where F
    fig = Plots.scatter(
...
function plot!(fig, vec::Vector{Point{F}}; colour=:blue) where F
    for i ∈ 1:length(vec)
