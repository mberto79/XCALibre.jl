...
- `rho` Density.
"""
struct Conduction{S1,F1,F2,S2,S3,F3,S4,C1,C2} <: AbstractEnergyModel
    T::S1
    Tf::F1
    rDf::F2            
    rhocp::S2
...
Energy{Conduction}(; material::Symbol, rho::Float64) = begin # maybe assign rho based on the material?
    args = (material, rho)
    ARGS = typeof(args)
...
(energy::Energy{EnergyModel, ARG})(mesh, solid) where {EnergyModel<:Conduction,ARG} = begin
    ...
    Conduction(T, Tf, rDf, rhocp, k, kf, cp, material, rho)
end


# Perhaps need to pass this as a list of arguments
function initialise(
    energy::Conduction, model::Physics{T1,F,SO,M,Tu,E,D,BI}, T_field, rDf, rhocp_field, k, kf, cp, rho, material, config
