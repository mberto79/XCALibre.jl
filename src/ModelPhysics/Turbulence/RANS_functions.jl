...
@kernel function _set_production!(
    ...
    if yplus > yPlusLam
        # Density-weighted, like every other term in the k equation
        # (Pk = rho*nut*S^2, Dkf = rho*beta*omega). This REPLACES Pk in the
        # wall cell, so without rho that cell alone gets a kinematic production
        # against a density-weighted sink. Invisible at rho = 1.
