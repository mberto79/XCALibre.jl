...
function turbulence!(
    ...
    @. arg1.values = min.(min.(max.(sqrt.(k.values)/(coeffs.β⁺*omega.values*y.values), 500.0*nu.values/(y.values*y.values*omega.values)), 4.0*coeffs.σω2*k.values/(CDkω.values*y.values*y.values)),10.0)
    @. F1.values = tanh.(arg1.values.^4)
    interpolate!(F1f, F1, config)

    @. arg2.values = min.(max.(2*sqrt.(k.values)/(coeffs.β⁺*omega.values*y.values), 500.0*nu.values/(y.values*y.values*omega.values)),100.0)
