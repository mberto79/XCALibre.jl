...
function setField_Box!(; mesh, field, value::F, min_corner::V, max_corner::V, config) where {F <: AbstractFloat, V <: AbstractVector}
    ...
    kernel! = _sized(_setField_Box!, backend, workgroup, ndrange)
...
function setField_Circle2D!(; mesh, field, value::F, centre::V, radius::F, config) where {F <: AbstractFloat, V <: AbstractVector}
    ...
    matched = KA.zeros(backend, Int64, ndrange)
    kernel! = _sized(_setField_Sphere!, backend, workgroup, ndrange)
...
function setField_Expression!(;
    ...
    matched  = KA.zeros(backend, Int64, ndrange)
    kernel!  = _sized(_setField_Expression!, backend, workgroup, ndrange)
