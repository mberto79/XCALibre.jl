...
RANS{KOmegaLKE}(; mesh, viscosity) = begin
...
struct KOmegaLKEModel{MK,MW,FK,FW,FN,C,S}
    k_eqn::MK
    ω_eqn::MW
    kf::FK
    ωf::FW
...
function initialise_RANSLKE(mdotf, peqn, config, model)
...
function turbulence!( # Sort out dispatch when possible
    ...
    (;k_eqn, ω_eqn, kf, ωf, νtf, coeffs, config) = KOmegaLKE
