...
function update_nueff!(nueff, nu, turb_model, config)
    ...
    if typeof(turb_model) <: Laminar
        kernel! = update_nueff_laminar!(backend, _workgroup(backend, workgroup, ndrange))
        kernel!(nu, nueff, ndrange=ndrange)
    else
        (; nutf) = turb_model
        kernel! = update_nueff_turbulent!(backend, _workgroup(backend, workgroup, ndrange))
        kernel!(nu, nutf, nueff, ndrange=ndrange)
    end
...
function flux!(phif::FS, psif::FV, config) where {FS<:FaceScalarField,FV<:FaceVectorField}
    ...
    ndrange = length(phif)
    kernel! = flux_kernel!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(phif, psif, ndrange=ndrange)
...
function flux!(phif::FS, psif::FV, rhof::FS, config) where {FS<:FaceScalarField,FV<:FaceVectorField}
    ...
    ndrange = length(phif)
    kernel! = _flux!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(phif, psif, rhof, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
end
...
function inverse_diagonal!(rD::S, eqn, config) where {S<:ScalarField}
    ...
    ndrange = length(rD)
    kernel! = _inverse_diagonal!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(rD, nzval, colval, rowptr, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
end
...
function correct_velocity!(U, Hv, ∇p, rD, config)
    ...
    ndrange = length(U)
    kernel! = _correct_velocity!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(U, Hv, ∇p, rD, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
end
...
remove_pressure_source!(U_eqn::ME, ∇p, config) where {ME} = begin # Extend to 3D
    ...
    ndrange = length(bx)
    kernel! = _remove_pressure_source!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(cells, source_sign, ∇p, bx, by, bz, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
end
...
function H!(Hv, U::VF, U_eqn, config) where {VF<:VectorField} # Extend to 3D!
    ...
    kernel! = _H!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(cells, cell_neighbours,
        nzval, rowptr, colval, bx, by, bz, U, Hv, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
end
...
max_courant_number!(cellsCourant, model, config) = begin
    ...
    kernel! = _max_courant_number!(backend, _workgroup(backend, workgroup, ndrange))
    kernel!(cellsCourant, U, runtime, mesh, ndrange=ndrange)
    # # KernelAbstractions.synchronize(backend)
