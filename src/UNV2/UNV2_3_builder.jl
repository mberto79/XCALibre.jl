...
function UNV2D_mesh(meshFile; scale=1.0, TI=Int64, TF=Float64)
    stats = @timed begin
    println("Loading mesh...")
    points, elements, boundaryElements = read_UNV2(meshFile, TI, TF);
    ...
    mesh = update_mesh_format(mesh, integer=TI, float=TF)
    end
