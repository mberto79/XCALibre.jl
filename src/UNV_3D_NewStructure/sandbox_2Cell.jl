unv_mesh="src/UNV_3D_NewStructure/tetra_2cell.unv"
...
#function generate_cell_faces(volumes)
    cell_faces=[]
    for n=1:length(volumes)
        #for i=1:length(volumes[n].volumes)
            push!(cell_faces,volumes[n].volumes)
            push!(store,cell_faces)
        #end
    end
   # return cell_faces
#end
...
cell_faces=Vector{Int}(generate_cell_faces(volumes))
...
function generate_cell_nodes_range(volumes)
    ...
    for i=1:length(volumes)
        ...
        start=4*i-3
        finish=4*i
        cell_nodes_range=UnitRange(start,finish)
...
function generate_faces_range(volumes,faces)
    ...
    @inbounds for i=1:length(volumes)
        start=4*i-3
        finish=4*i
        cell_faces_range=UnitRange(start,finish)
