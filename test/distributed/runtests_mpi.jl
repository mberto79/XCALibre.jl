...
#   julia --project test/distributed/runtests_mpi.jl [--ranks=1,2,3,5] [testfile...]
# Eight ranks on fewer cores need no launcher flag with the stock (hydra) mpiexec: --ranks=8.
include(joinpath(@__DIR__, "driver.jl"))

args = copy(ARGS)
i = findfirst(startswith("--ranks="), args)
ranks = i === nothing ? [1, 2, 3, 5] : parse.(Int, split(split(popat!(args, i), '=')[2], ','))
