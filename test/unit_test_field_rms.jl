...
postprocess = FieldRMS(model.momentum.U;name="U_rms",start = start*timestep ,update_interval = 2*timestep)
...
u_rms = sum(postprocess.rms.x.values[end-25:end-15])/length(postprocess.rms.x.values[end-25:end-15])
v_rms = sum(postprocess.rms.y.values[end-25:end-15])/length(postprocess.rms.y.values[end-25:end-15])
